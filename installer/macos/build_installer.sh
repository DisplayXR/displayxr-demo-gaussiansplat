#!/usr/bin/env bash
# Build the macOS .pkg for the Gaussian Splat demo.
#
# Usage:
#   DISPLAYXR_VERSION=1.4.0 ./build_installer.sh <artifact-dir> [output.pkg]
#
# <artifact-dir> must contain:
#   bin/gaussian_splatting_handle_vk_macos        the cmake-built binary
...
#   assets/butterfly.spz                          bundled default scene
#   displayxr/...                                 app manifest + icons
#
# Two-stage build (mirroring displayxr-runtime/installer/macos/build_installer.sh):
#   1. pkgbuild --root <staging>  → gaussiansplat.pkg   (component .pkg)
#   2. productbuild --distribution Distribution.xml ... → DisplayXRGaussianSplat-<v>.pkg
#
...
OUTPUT_PKG="${2:-$(pwd)/DisplayXRGaussianSplat-${VERSION}.pkg}"
...
echo "==> Building DisplayXRGaussianSplat installer"
...
# We want the .app at /Applications/Gaussian Splat Viewer.app, so the
...
( cd "$APPS_DIR" && DISPLAYXR_VERSION="$VERSION" \
    bash "$SCRIPT_DIR/create_app_bundle.sh" "$ARTIFACT_DIR" "Gaussian Splat Viewer.app" )
...
COMPONENT_PKG="$WORK_DIR/gaussiansplat.pkg"
pkgbuild \
    --root "$PAYLOAD_ROOT" \
    --identifier com.displayxr.gaussiansplat \
    --version "$VERSION" \
    --install-location / \
    "$COMPONENT_PKG"
