#!/bin/bash
# Create a macOS .app bundle for the Gaussian Splat demo.
#
...
#  - The bundled butterfly.spz scene is copied next to the binary (matches
...
#                   assets/butterfly.spz + displayxr/{manifest.json,icons}.
set -e

ARTIFACT_DIR="${1:?Usage: $0 <artifact-dir> [output.app]}"
APP_BUNDLE="${2:-Gaussian Splat Viewer.app}"
BINARY_NAME="gaussian_splatting_handle_vk_macos"
VERSION="${DISPLAYXR_VERSION:-1.0.0}"

BUNDLE_DISPLAY_NAME="Gaussian Splat Viewer"
BUNDLE_ID="com.displayxr.gaussiansplat"
...
cd "$DIR"
exec "$DIR/gaussian_splatting_handle_vk_macos" "$@"
LAUNCHER
...
if [ -f "$ARTIFACT_DIR/assets/butterfly.spz" ]; then
    cp "$ARTIFACT_DIR/assets/butterfly.spz" "$APP_BUNDLE/Contents/Resources/"
fi
