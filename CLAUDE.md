...
`displayxr-common` v2.11.0's `EndFrame`/`EndFrameWithWindowSpaceLayers` have no parameter to reach `XrFrameEndInfo::next` (only `projectionNext`, which chains onto `XrCompositionLayerProjection::next` — a different chain the runtime does not read `XrContentBoundsDXR` from). Until the shared helpers grow a `frameEndNext` parameter, `windows/main.cpp` calls two local shims (`EndFrame_ChainFrameEndNext` / `EndFrameWithWindowSpaceLayers_ChainFrameEndNext`, defined just above `RenderThreadFunc`) that are byte-for-byte mirrors of the v2.11.0 helpers plus that one field — delete them and switch back to the shared calls once that parameter exists upstream. HUD appends `roi u0,v0-u1,v1` to the `rear:` line whenever a bounds rect was computed this frame.
