...
**Rear depth budget (`XR_DXR_depth_budget`, #100).** `far_offset` above (the `1000·vH` unrestricted / `0` foreground-clip choice) is no longer hand-rolled per call site — it's resolved by `dxr::ResolveClipPlanes(ez, vH, budget, transparent, standalone)` (`displayxr-common`'s `common/clip_policy.h`), called once per eye in `windows/main.cpp` (near the `xrLocateViews` site) and again at the double-click pick site so both windows agree. `budget` is the runtime's advisory, time-ramped `XrRearDepthBudgetDXR` — chained onto `XrViewState::next` via `dxr::ChainRearDepthBudget` only when the runtime advertises `XR_DXR_depth_budget` (`XrDepthBudgetExtAvailable()`); `nullptr` (extension absent, or the app didn't enable it) reproduces the old `far_offset = (transparent && standalone) ? 0 : 1000` rule bit-for-bit. The runtime — not this app — decides *when* the far plane may open behind the ZDP (a neutral desktop behind the window) and ramps it smoothly; the app applies `farOffsetVH`/`clipFar` as-is and adds no smoothing of its own. HUD shows the live state as `rear: <state> <farOffsetVH>vH` via `dxr::RearDepthBudgetStateName`.

**Content-bounds ROI (`XR_DXR_depth_budget` SPEC_VERSION 2, #100 v2).** v1 analysed the whole canvas for a horizontal-disparity cue, which could needlessly close the budget on busy pixels nowhere near the model (e.g. an empty Notepad window's menu/status-bar edges). Every frame, `windows/main.cpp` projects the cached, outlier-trimmed scene AABB (`GsRenderer::getSceneBBox`, cached once per load in `ApplyAutoFitForLoadedScene_locked` — it's an O(N) scan, not something to redo per frame) through each rendered eye's column-major `proj * view` (`view_rig_math.h`'s `vrm_mat4_multiply`; this app's matrices are already GL-convention column-major with NDC y up — `mat4_from_xr_fov`'s `m[11]=-1` — the exact convention `dxr::ProjectAabbToCanvasBounds` expects, so no transpose/flip is needed) into a canvas-normalised union rect, then chains it as `XrContentBoundsDXR` (`dxr::ChainContentBounds`, `displayxr-common`'s `common/content_bounds.h`) onto the frame's `XrFrameEndInfo::next`. No scene loaded → nothing is chained → the runtime falls back to the v1 whole-canvas ROI.

**Content-mask ROI (`XR_DXR_depth_budget` SPEC_VERSION 3) and where its silhouette comes from (#112 / v4).** When the runtime advertises v3, `windows/main.cpp` also chains `XrContentMaskDXR` — an occupancy grid, i.e. the *silhouette* rather than v2's rect. **It is NOT built from the rendered alpha.** It used to be (`g_punch.coverage()`, the same readback that shapes the click-through window region), and that closed a feedback loop: the far clip here is a **per-splat cull in the compute preprocess stage** (`3dgs_common/shaders/preprocess.comp`, mirrored in `adreno_preprocess.comp`), so the rendered silhouette is a smooth monotone function of the budget the runtime published last frame — the runtime then measured the background it had just caused to be revealed and the rear clip oscillated every ~0.6–1.1 s over a text/blank border (#112, runtime#1470). Spec v4 requires the silhouette **as it would render at unrestricted budget**.

So the two artefacts diverge deliberately: `SetWindowRgn` keeps the **clipped** alpha (that one *is* a visual clip — the window must not stay clickable where nothing is drawn), and the mask comes from `GsRenderer`/`GsAdrenoRenderer::readSilhouetteCoverage()`. Both preprocess shaders scatter each surviving gaussian's screen bbox into a small `R8_UINT` storage image (~1 texel per 16×16 render px, ≤128 per side, ~8 KB) **before** the `clip_far` cull — no second dispatch, no second sort, no second render pass; the far cull simply moved below the scatter, and since every test in between is a plain `return` and all `attr[]` writes sit past the last cull, the rendered image is unchanged. The readback clears the image in the same submission, so it holds exactly the union over that frame's views. **Armed per session** (`setSilhouetteCoverage`), never per clip state: a coverage source that changed shape when the clip engaged would jitter on its own. Unavailable (unarmed, no scene, readback failed) → nothing is chained and the runtime falls back to the clip-independent v2 bounds. Android never arms it (no background preview; the budget is pinned clipped), so that path is bit-for-bit unchanged. Exercise the raster on macOS/MoltenVK with `DXR_GAUSS_MASKCOV_TEST=1`, which dumps coverage % plus an ASCII silhouette.

`displayxr-common` v2.11.1's `EndFrame`/`EndFrameWithWindowSpaceLayers` take a trailing `const void* frameEndNext = nullptr` parameter that is written straight to `XrFrameEndInfo::next` — a different chain from `projectionNext` (which chains onto `XrCompositionLayerProjection::next`, not read by `XrContentBoundsDXR`). `windows/main.cpp` passes the chained `XrContentBoundsDXR` pointer (or `nullptr` when the extension is off / no scene is loaded) as that final argument to the shared helpers directly — no local shim needed. HUD appends `roi u0,v0-u1,v1` to the `rear:` line whenever a bounds rect was computed this frame.
