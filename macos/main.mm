...
struct AppXrSession {
    ...
    // (top-down, origin = primary top-left) from XrDisplayDesktopPositionDXR
    ...
    // XR_DXR_mcp_tools (#32): viewer controls exposed as agent tools on the
    ...
    // XR_DXR_view_rig (W7 of #396): the runtime owns the off-axis Kooima math
...
// ============================================================================
// Agent tools (XR_DXR_mcp_tools, #32)
// ============================================================================
//
...
// XrEventDataMCPToolCallDXR through PollEvents() — on the main loop, where
...
static bool InitializeOpenXR(AppXrSession& xr) {
    ...
    if (xr.hasDisplayInfoExt) {
        ...
        XrDisplayDesktopPositionDXR dp = {};
        dp.type = XR_TYPE_DISPLAY_DESKTOP_POSITION_DXR;
        ec.next = &dp;
    ...
    // XR_DXR_mcp_tools (#32): resolve the agent-tool entry points.
    if (xr.hasMcpToolsExt) {
        xrGetInstanceProcAddr(xr.instance, "xrSetMCPAppInfoDXR", (PFN_xrVoidFunction*)&xr.pfnSetMCPAppInfoEXT);
        xrGetInstanceProcAddr(xr.instance, "xrRegisterMCPToolDXR", (PFN_xrVoidFunction*)&xr.pfnRegisterMCPToolEXT);
        xrGetInstanceProcAddr(xr.instance, "xrGetMCPToolCallArgsDXR", (PFN_xrVoidFunction*)&xr.pfnGetMCPToolCallArgsEXT);
        xrGetInstanceProcAddr(xr.instance, "xrSubmitMCPToolResultDXR", (PFN_xrVoidFunction*)&xr.pfnSubmitMCPToolResultEXT);
...
static bool CreateSession(AppXrSession& xr, VkInstance vkInstance, VkPhysicalDevice pd,
    ...
    // XR_DXR_mcp_tools (#32): declare identity + register the agent tools.
...
static void PollEvents(AppXrSession& xr) {
    ...
    while (xrPollEvent(xr.instance, &event) == XR_SUCCESS) {
        if (event.type == XR_TYPE_EVENT_DATA_SESSION_STATE_CHANGED) {
            ...
            // Agent invoked one of our registered tools (XR_DXR_mcp_tools).
...
int main(int argc, char** argv) {
    ...
    // Otherwise take the runtime's active 3D mode (gated on 3D; see
    // AppXrSession::activeRenderingMode). When nothing 3D is active and nothing
    // is pinned, the old default (mode 1, the first 3D mode) stands. Later
            ...
            if (BeginFrame(xr, frameState)) {
                ...
                if (frameState.shouldRender) {
                    ...
                    if (XR_SUCCEEDED(locResult) &&
                        ...
                        // runtime#1486 INV-3.1: eyeCount is what gets RENDERED (the
                        // layer itself carries every located view — ADR-041, see
                        // the projectionViews sizing below), so it is the single place to reconcile the counts that can
                        ...
                        if (AcquireSwapchainImage(xr, imageIndex)) {
                            ...
                            // view; only [0, eyeCount) are rendered and the tail is
                            // aliased onto view 0 right after the fill loop. Under
                            // PRIMARY_MULTIVIEW_DXR xrEndFrame refuses anything
                            // shorter, so a 1-view (2D) frame submitted as one view was
                            // dropped outright — the panel went 2D while still showing
                            // the last woven 3D frame. The max() only matters if the
                            // locate returned 0 views (eyeCount is floored at 1).
                            projectionViews.assign(
                                (size_t)(runtimeViewCount > (uint32_t)eyeCount ? runtimeViewCount
                                                                               : (uint32_t)eyeCount),
