...
gs_init()
{
	...
	// Render-scale and load-time decimation are owned by GsAdrenoRenderer itself
	// (Adreno-tuned defaults — render_scale 0.45, keep 0.25 — plus its own
	// debug.dxr.gs.scale / debug.dxr.gs.keep property reads in init()). The
	// graphics renderer self-tunes, so main.cpp no longer injects these knobs;
	// the compute-era setRenderScale/setCullMinOpacity/setKeepFraction setters
	// don't exist on GsAdrenoRenderer and would override its tuning.
	g_gs_ready = true;
	LOGI("GsRenderer initialized (%ux%u/eye)",
