 ...
 *   @ c1e4fe00da0f189122eca14e61e9faaa88e4b38e   (verbatim, plus the
 *   second-stage #ifndef guard at the bottom of the include block below)
 *
 * It deliberately lives at the TOP level of openxr_includes/, not inside
 * openxr_includes/openxr/: it is a DisplayXR app helper, not a header from the
 * OpenXR registry, and openxr_includes/openxr/ is the byte-pinned vendor dir
 * that scripts/check_vendored_headers.py + VENDORED.json govern. This one is
 * pinned in prose, here, because it comes from a different runtime path than
 * VENDORED.json's single source_path.
 *
 * openxr_includes/ is the one directory on ALL four legs' include paths
 * (windows/, macos/, linux/, android/), which is why it is here -- every leg
 * reaches it as `#include <dxr_view_config.h>`.
 *
 * displayxr-common v2.14.0 will ship the same helper; once this repo re-pins
 * common/CMakeLists.txt to it, the Windows leg can drop the include and this
 * file stays only for the three legs that carry their own session code.
 * ---------------------------------------------------------------------------
 */
...
/*
 * Second stage (vendored-copy addition, not in the runtime original).
 *
 * The probe above prefers the header, but this repo vendors its own
 * openxr_includes/openxr/XR_DXR_display_info.h -- so the header can be PRESENT
 * and still be older than SPEC_VERSION 19, in which case the include above
 * defines nothing and the #else never runs. Without this, a stale vendored
 * header breaks the build outright instead of degrading to the probe.
 *
 * It only fires below spec 19, so it cannot shadow a future openxr.h that
 * supplies the enumerator for real (our vendored display_info.h is >= 19 by
 * then). Value is fixed by the DXR author-ID block either way; the
 * xrEnumerateViewConfigurations probe below is what actually decides whether
 * the RUNTIME has it.
 */
#if !defined(XR_DXR_display_info_SPEC_VERSION) || XR_DXR_display_info_SPEC_VERSION < 19
